...
app.post('/add-user', authenticateToken, (req, res) => {
  const {vehicleId, vehicleType, tollgateId, tollFee } = req.body;
  ...
  db.query(query, [vehicleId, vehicleType, tollgateId, tollFee], (err, results) => {
    if (err) {
...
app.get('/check-fare', authenticateToken, (req, res) => {
  ...
  db.query(query, [tollgateId, date], (err, results) => {
    ...
    const vehicles = results[1]; // Assuming results[1] contains vehicle data
    const totalFare = results[0][0] ? results[0][0].total_fare : 0; // Assuming results[0] contains total fare
    const vehicleCount = vehicles.length;
